...
do
  # echo "${scene_data##*/}"
  OMP_NUM_THREADS=12 CUDA_VISIBLE_DEVICES=1 python ../train_marepo.py \
  ...
  --head_network_path "../logs/${scene_data##*/}/${scene_data##*/}.pt" \
  --use_half=False \
  --batch_size 8 \
  --trainskip 1 \
  --testskip 1 \
...
for scene in ${DATASET_PATH_TEST}; do
  ...
  CUDA_VISIBLE_DEVICES=1 python $testing_exe "${scene}" "$marepo_head_path" --head_network_path ${ace_head_path} \
